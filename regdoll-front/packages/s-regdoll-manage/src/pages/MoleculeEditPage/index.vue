 ...
 * @Editors: lgldlk
 * @LastEditTime: 2021-07-16 21:32:09
-->
<template>
  <div class="moleculeEditPage">
    <div id="container"></div>
    <div>
      <router-link class="goHome"
                   to="/">
        ...
        <div class="moleculeCon drawer-inside">
          ...
          <div class="">
            <el-button type="primary"
                       plain
                       @click="saveMolecule">保存</el-button>
          </div>
        </div>
      </el-drawer>
    </div>

    <div class="show_atom_name"
         v-show="nowSelectObj != undefined"
         @clisk="openAtomChangeName">
      {{ nowSelectObj?.atom.en_name }}&nbsp;&nbsp;{{ nowSelectObj?.valence }}&nbsp;&nbsp;&nbsp;{{ selectAtomPosition.x + "," + selectAtomPosition.y + "," + selectAtomPosition.z }}
    </div>
    <!-- <popWindow
    ...
    <popWindow v-if="valenceInputVis"
               windowTitle="原子化合价输入"
      ...
      <div>
        <div class="">
          化合价：
          <el-input-number size="mini"
                           v-model="valenceInput"></el-input-number>
        </div>
      </div>
      <template #footer>
        <span class="popWindow_footer">
          <el-button size="mini"
                     :round="true"
                     @click="closeValence">取 消</el-button>
          <el-button size="mini"
                     type="primary"
                     round="round"
                     @click="setSelectAtomValence">确 定</el-button>
        </span>
      </template>
    </popWindow>
    ...
    <popWindow v-if="atomPositionWindow"
               windowTitle="坐标输入"
               @forkClick="closeAtomPositionWindow">
      <div>
        <div class="">
          X:
          <el-input v-model="selectAtomPosition.x"
                    placeholder="请输入x轴坐标"></el-input>

          Y:
          <el-input v-model="selectAtomPosition.y"
                    placeholder="请输入y轴坐标"></el-input>
          Z:
          <el-input v-model="selectAtomPosition.z"
                    placeholder="请输入z轴坐标"></el-input>
        </div>
      </div>
      <template #footer>
        <span class="popWindow_footer">
          <el-button size="mini"
                     :round="true"
                     @click="closeAtomPositionWindow">取 消</el-button>
          <el-button size="mini"
                     type="primary"
                     round="round"
                     @click="changeSelectAtomPosition">确 定</el-button>
        </span>
      </template>
    </popWindow>
